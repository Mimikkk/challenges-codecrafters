...
import { CatFileCommand } from "./CatFileCommand.ts";
import { HashObjectCommand, HashObjectCommandErrorNs } from "./HashObjectCommand.ts";

describe("Git Commands - HashObjectCommand", () => {
  useSilentLogger();
  const filesystem = useFileSystems("git-commands-hash-object");
  ...
  it("should return error for non-existent file", () => {
    ...
    const hash = HashObjectCommand.dispatch({
      manager: GitObjectManager.new(fs),
      filesystem: fs,
      options: { path: "missing.txt" },
      parameters: [],
    });

    expect(hash).toEqual(Result.err(HashObjectCommandErrorNs.notFound("missing.txt")));
  });
});
